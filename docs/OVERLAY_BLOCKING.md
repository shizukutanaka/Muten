...
| forced_retention_cue | +20 | `alert_shaped` AND normalized title contains a "do not close / do not turn off / keep this window open / stay on this page" instruction. High-specificity: legitimate software never puts a retention instruction in a window title. |
| credential_harvest_cue | +20 | `alert_shaped` AND normalized title contains account-alarm language (account suspended/locked/compromised, unusual/suspicious sign-in/login/activity) OR credential-entry instructions (verify account, confirm password, enter credentials, update payment). Phishing-overlay pattern for bank/email/social credential theft. |
| fake_scanner_cue | +20 | `alert_shaped` AND normalized title contains fake rogue-AV scan-progress language: scanning for threats/viruses/malware, N threats detected/found, removing malware, system repair in progress, critical system error detected. |
...
| authority_lure | +25 | `alert_shaped` AND normalized title contains an LEA agency token (fbi, interpol, cybercrime, homeland security, law enforcement…) + a coercion token (warning, notice, locked, illegal, fine, arrested). Covers Reveton/Winlock-style ransomware-bluff overlays. |
