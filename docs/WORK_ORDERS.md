...
| Blocklist loader drops or **reinterprets** mis-authored rules with no diagnostic (DR-22) | a hot-reloaded, operator-edited rule that fails to load is an unannounced detection hole; a mis-cased `Title:` silently becomes a *host* rule | **WO-13** (shell linter shipped; since DR-30 the shipped file is also proven against the real loader — the runtime diagnostic for operator edits is still open) |
